...
source "proxmox-clone" "ubuntu-docker-registry-pve3" {
...
source "proxmox-clone" "ubuntu-docker-registry-pve2" {
  # Proxmox Connection Settings
  proxmox_url              = var.proxmox_api_url
  insecure_skip_tls_verify = var.proxmox_insecure_url
  token                    = var.proxmox_api_token_secret
...
source "proxmox-clone" "ubuntu-docker-registry-pve1" {
  # Proxmox Connection Settings
  proxmox_url              = var.proxmox_api_url
  insecure_skip_tls_verify = var.proxmox_insecure_url
  token                    = var.proxmox_api_token_secret
...
build {
  sources = ["source.proxmox-clone.ubuntu-docker-registry-pve1","source.proxmox-clone.ubuntu-docker-registry-pve2","source.proxmox-clone.ubuntu-docker-registry-pve3"]
 
  provisioner "shell" {
    environment_vars = [
      "DOCKER_USER=${var.ssh_username}",
    ]
    execute_command = "chmod +x {{ .Path }}; sudo {{ .Vars }} {{ .Path }}"
    script          = "./scripts/docker-registry-install.sh"
    timeout         = "15m"
  }
